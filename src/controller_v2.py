...
class Node:
    ...
    def split_curve_2(self, points_x, points_y):
        ...
        joint_pos = np.zeros([self._link_N + 1, 2])     # Create joint positions matrix

        # save the position of the true head
        joint_pos[0, 0] = points_x[-1]
        joint_pos[1, 0] = points_y[-1]

        # add head direction points for the spline
        head_dir = np.asarray([[500], [0], [1]])
        p_new = self.A_head * head_dir
        points_x_new = np.append(points_x, np.linspace(points_x[-1], p_new[0,0], 100))
        points_y_new = np.append(points_y, np.linspace(points_y[-1], p_new[1, 0], 100))

        # reverse the points to be end->start, so the spline u=0 will be at the end
        points_x_new = np.fliplr(points_x_new)
        points_y_new = np.fliplr(points_y_new)

        # Create spline
        tck, U = sc.splprep([points_x_new, points_y_new], s=0)
        # new_points = sc.splev(u_points, tck)
        # start_points = sc.splev(0, tck)
        # end_points = sc.splev(1, tck)
        ...
        for i in range(self._link_N):
            ...
            while error >= epsilon:
                counter += 1
                c = (a + b) / 2
                # p_temp = sc.splev(c, tck)
                p_temp = np.reshape(sc.splev(c, tck), (2, 1))           # calc temp point along the spline
                dist = np.linalg.norm(joint_pos[:, i] - p_temp)         # calc the distance from the temp point to the previous joint position
            ...
            # print('Link #{:d}:\titer: {:d}\terror dist={:.7f}'.format(i + 1, counter, error))
            # print("Link #", i + 1, "iter", counter, "dist", dist)
        ...
        joint_angles = np.asarray([first_angle])
        joint_angles = np.append(joint_angles, self.calc_angle(joint_pos))

        joint_cmd = np.array([joint_pos[0, 0], joint_angles])

        # Print log to terminal
        msg_info = "Average iterations per link = {.1d}".format(counter/self._link_N)
    ...
    def head_move(self, state):
        if state > _joy_dead_zone:
            ...
            for i in range(self._link_N+1):
                pub_vec[i].publish(Float64(data=joints_angles[i]))
